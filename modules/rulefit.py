# stable rulefit version
# We change set() by numpy to remove randomness
...
class MySet:
    def __init__(self, ls=[]):
        self.set = np.array(ls)
        self.set = np.unique(self.set)

    def add(self, item):
        self.set = np.append(self.set, item)
        self.set = np.unique(self.set)

    def update(self, values):
        for v in values:
            self.set = np.append(self.set, v)
...
def extract_rules_from_tree(tree, feature_names=None, seeds=None):
    ...
    def traverse_nodes(node_id=0,
        ...
        if tree.children_left[node_id] != tree.children_right[node_id]: 
            ...
            if len(new_conditions)>0:
                ...
                # rules.append(new_rule) # rules.update([new_rule])
            else:
...
class RuleEnsemble():
    ...
    def _extract_rules(self):
        ...
        for tree in self.tree_list:
            rules = extract_rules_from_tree(tree=tree[0].tree_, feature_names=self.feature_names)
...
class RuleFit(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None, feature_names=None):
        ...
        if 'r' in self.model_type:
            ...
            tree_list = self.tree_generator.estimators_ # already test constant
            ...
            ## extract rules # not stable here, need to be fixed
        ...
        if self.rfmode=='regress':
            ...
            self.lscv=LogisticRegressionCV(Cs=Cs,cv=self.cv,penalty='l1',random_state=self.random_state,solver='liblinear', max_iter=300000) # increase by 300000
            self.lscv.fit(X_concat, y)
