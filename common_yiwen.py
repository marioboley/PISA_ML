...
class Splitdata:
    ...
    def __init__(self, data, points, rep=1, frac=None, n=None, seed=None):
        ...
        datax: x
        points: mathed condition data
        rep: repeation
        frac: fraction of data included in training data
        n: num of data included in training data
        seed: set seed if run split function, keep seed=None
        """
        self.data = data
        self.point = points
        self.rep = rep
    ...
    def __split__(self):
        if self.frac != None:
            frac = 1- self.frac
            test_index = self.point.sample(frac=frac, random_state=self.seed).index
        if self.n != None:
            test_index = self.point.sample(n=len(self.point)-self.n, random_state=self.seed).index
        X_test = self.data.loc[test_index,] if self.n or self.frac else self.point
        X_train = self.data[~self.data.index.isin(test_index)]
        return [list(X_train.index), list(X_test.index)]
    
    def get_n_splits(self, x=None, y=None, groups=None):
        """
        x, y, groups are just default, we don't use any of them
        """
        return self.rep
...
class Experiment:
    ...
    def __init__(self, estimators, estimator_names, splitter, x, y, groups=None, evaluators=['accuracy'], verbose=True):
        ...
        self.num_reps = self.splitter.get_n_splits(self.x, self.y) # set it constant fix later with kfolder
        self.results_ = None
        self.fitted_ = None
