...
class Experiment:
    ...
    def run(self):
        ...
        for train_idx, test_idx in self.splitter.split(new_x, new_y, new_group):
            ...
            num += 1
            dic['num'].append(num)
            dic['train'].append(str(list(train_idx))[1:-1]) # [1:-1] to remove "[" & "]"
            dic['test'].append(str(list(test_idx))[1:-1])
        ...
        if self.file_name:
            df = pd.DataFrame(dic)
            df.to_csv(self.file_name + '_index.csv', index=False)
        if self.verbose:
            print()
...
class ExtrapolationExperiment(Experiment):
    ...
    def __init__(self, estimators, estimator_names, x, y, groups, score=['accuracy', sample_size], verbose=True, extrapolation_index=None, min_test_size=None, file_name=None):
        Experiment.__init__(self, estimators, estimator_names, GroupKFold(len(set(groups))), x, y, groups, score, verbose, extrapolation_index, min_test_size, file_name)
